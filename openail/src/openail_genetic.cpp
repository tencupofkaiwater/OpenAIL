...
#include <openail/inc/openail_genetic.h>
...
void Genetic::initialpopulation(int ps, int gen, double cr,
    double mr, double *xtop, double *xbottom)  //第一步，初始化。
{
    ...
    { 
        ...
        current.fitness=0;
        current.value=0;
        population.InsertAfter(population.FindIndex(i),current);//除了初始化使用insertafter外,其他的用
    }
}

void Genetic::generatenextpopulation()//第三步，生成下一代。
{
...
//void Genetic::evaluatepopulation()   //第二步，评价个体，求最佳个体
//{
...
void Genetic:: calculateobjectvalue()  //计算函数值，应该由外部函数实现。主要因为目标函数很复杂。
{
    int i,j;
...
void Genetic::selectionoperator()   
{
...
void Genetic:: findbestandworstindividual( )  
{
    int i;
...
void Genetic:: calculatefitnessvalue() 
{
    //适应度函数值计算，关键是适应度函数的设计
...
void Genetic:: performevolution() //演示评价结果,有冗余代码,current变化，程序应该改变较大
{
    if (bestindividual.fitness>currentbest.fitness)
    {
...
void Genetic::GetResult(double *Result)
{
    int i;
    for (i=0;i<variablenum;i++)
    {
...
void Genetic::GetPopData(CList <double,double>&PopData)  
{
    PopData.RemoveAll();
    int i,j;
    for (i=0;i<popsize;i++)
...
void Genetic::SetFitnessData(CList <double,double>&PopData,
    CList <double,double>&FitnessData,
    CList <double,double>&ValueData)
{
    int i,j;
    for (i=0;i<popsize;i++)
    {  
